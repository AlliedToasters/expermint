...
class UniqueIDMaker(object):
    ...
    id.
    """
    def __init__(self, n_chars=5, other_ids=set()):
        self.n_chars = n_chars
        self.issued = set(other_ids)
...
class BatchMaker(object):
    ...
    def make_batch(self, images):
        ...
        images = [x.astype(float) for x in images]
        for img in images:
    ...
    def unmake_batch(self, batch):
        """Takes a batch and corrects the color channels."""
        images = [batch[x, :, :, :] for x in range(batch.shape[0])]
        images = [(x*255) for x in images]
        for img in images:
            img[:, :, 0] = img[:, :, 0] + self.c0m
            img[:, :, 1] = img[:, :, 1] + self.c1m
...
def build_df(people):
    """Builds a df given input people. Returns df."""
    df = pd.DataFrame()
    ids = list(people.keys())
    values = [(x.sex, x.age, x.race) for x in list(people.values())]
    df['unique_id'] = ids
    df['sex'] = [v[0] for v in values]
    df['age'] = [v[1] for v in values]
    df['race'] = [v[2] for v in values]
    return df
...
def make_features(df, batch_maker, save=True):
    ...
    for i, row in df.iterrows():
        ...
        templates[row.unique_id] = template
        if save:
...
def view_all_imgs(df, idx):
    ...
    fig.suptitle('identity {}'.format(df.loc[idx].unique_id))
    skip = False
    try:
        no_axes = len(axes)
    except:
...
def sample_by_subgroups(df, age_groups=[(18, 40),(40,)], n_class=2622, seed=None):
    ...
    for group in age_groups:
        ...
        groups.append(grp)
    sexes = ['male', 'female']
    races = ['asian', 'black', 'caucasian', 'indian', 'hispanic']
    classes = []
    sample_no = int(n_class/(len(sexes)*len(races)*len(age_groups)))
    for sex in sexes:
        for race in races:
...
def make_image_batch(df, people, idxs, class_coder, batch_maker, train=False):
    """Returns a batch of given indices"""
    slc = df.loc[idxs]
    lbls = list(slc.unique_id)
    X = []
    Y = []
    for i, lbl in enumerate(lbls):
        img = people[lbl].imgs[0]
        if train:
            img = augment(img)
        elif not train:
            img = flip(img)
...
def datagen(df, people, batch_maker, batch_size=64, train=False, do_shuffle=False):
    """Generates data."""
    class_coder = dict()
    cnt = -1
    for i, row in df.iterrows():
        cnt += 1
        class_coder[row.unique_id] = cnt
        class_coder[cnt] = row.unique_id
    yield class_coder
    batch = []
    while True:
        if do_shuffle:
        ...
        for i, row in df.iterrows():
            ...
            if len(batch) == batch_size:
                X, Y= make_image_batch(df, people, batch, class_coder, batch_maker, train=train)
                yield X, Y
                batch = []
...
def perform_experiment(df, batch_maker, people, ticker=0):
    ...
    x = VGGFace(include_top=False, model='resnet50', input_shape=(224, 224, 3))(inputs)
    x = Flatten()(x)
    out = Dense(
        n_class, 
        activation='softmax'
    ...
    train_gen = datagen(df, people, batch_maker, batch_size=batch_size, train=True, do_shuffle=True)
    class_coder = next(train_gen)
    val_gen = datagen(df, people, batch_maker, batch_size=batch_size, train=False)
    next(val_gen)
